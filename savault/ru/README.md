...
###### AES-256-CBC
Данные этого метода шифрования занимают 16 байт
...
data {
    ...
    cipher: {                     // Существует только если encrypted=01
        ...
        uses:     [1 байт     ],  // Что использовалось для шифрования данных: мастер-пароль, ключ-файл, физ.ключ или USB-ключ
        hash:     [32 байта   ],  // SHA256-хеш первых 32 байт дешифрованных данных
...
Первое поле `encrypted` занимает 1 байт и указывает на то, зашифрованы ли далее идущие данные.
Если это поле равно `00`, то данные не зашифрованы и можно переходить к **их обработке.**
Если же его значение равно `01`, то после идут данные шифровщика, с помощью которых нужно дешифровать их.

Следующее поле - `cipher`, как было сказано ранее, указывает на то, как надо дешифровать данные.
Его первый байт (`method`) указывает на используемый метод:

Значение `method` | Описание                  | Рекомендуется
----------------- | ------------------------- | -------------
`00` (0)          | Указывает на конец циклов |
`01` (1)          | `AES-256-CBC`             | ✅
...
`03` (3)        | 2-й тип USB-ключа passtect'а  | `pt_usb_key.value`

После идут 32 байта (`hash`) - SHA256-хеш первых 32 байт дешифрованных данных (`random`). Оно нужно для проверки правильности вводимых данных.

После идёт 2 байта (`length`) - целочисленное `big-endian` число. Это длина далее идущих данных шифровщика (`data`).
...
2. Разжать данные согласно методу сжатия (заголовок с `02` ID)

После можно приступать к обработке. Те данные, что у нас остались - это кучка блоков:
```js
block {
...
Длина и содержание данных (`data`) зависит от типа блоков. Логи (`logs`) - это кучка элементов описаных ниже. А дочерние блоки (`child`) - это точно такая же кучка блоков.
...
Все логи начинаются N байт типа (`type`) - UTF-8 строка, оканчивающаяся на `\x00`. Тип `end` указывает на конец записей журнала.

Далее идёт 8 байт даты (`date`) и времени создания записи журнала. Это `Timestamp64`. 
...
**attributes**  | `List[Attr]`  |              | Ниже                                   | Аттрибуты записи
...
Первое поле аттрибута - это UUID, оно наследуется от UUID того же аттрибута у шаблона. Аттрибут с UUID `00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00` указывает на конец списка аттрибутов. 

Второе поле - это название (`name`) - UTF-8 строка, оканчивающаяся на `\x00`. Пустое значение (не учитывая `\x00`) обозначает, что название нужно наследовать от того же аттрибута у шаблона.

Далее идёт логическое (`0` - `false`; `1` - `true`) поле `protected`, указывающие на то, защищены ли данные аттрибута. Может иметь значение `02`, тогда значение наследуется от того же аттрибута у шаблона.

После идёт поле длины (`length`) - целочисленное `big-endian` число. Оно указывает на длину значение аттрибута.

Последнее поле - `value` - имеет длину, указанную в поле `length`. Если значение поля `protected` - это `0` (`00`), то дополнительно дешифровать значение не нужно. Если же оно имеет значение `1` (`01`), то сначала нужно дешифровать его, согласно данным из метаданных.
