...
--------------------------------------------------- | ------------------------------------- | -------------
*Прошлые версии убраны по причине недоработанности* |                                       |
...
`03` (3)     | 32 байта               | SHA256-хеш                    | ❌
`04` (4)     | 16 байт                | PUID[^2]3[^5] базы данных     | ✅

#### Название базы данных (`01`)
Значение этого заголовка - это `UTF-8` строка. Используется для отображения хоть какого-то названия базы данных в интерфейсе. Настоящее же название указывается в зашифрованных метаданных.
...
#### PUID базы данных (`04`)
Значением этого заголовка является PUID[^2], основанный на времени[^5]. Данный заголовок нужен для обозначения базы данных в некоторых частях проекта Piema. Например, в протоколах или ключах доступа.
...
**recyle_bin**   | `PUID`        | 16           | PUID                        | PUID[^2][^3] группы корзины, если её нет, то нулевой[^4]
...
Название поля   | Тип           | Длина, байты | Метод хранения                         | Описание
--------------- | ------------- | ------------ | -------------------------------------- | ----------------------------------------------
**puid**        | `PUID`        | 16           | PUID[^2]                               | PUID[^2] записи
**template**    | `PUID`        | 16           | PUID[^2]                               | PUID[^2] записи-шаблона
**name**        | `String`      |              | UTF-8 строка + `\x00`                  | Название записи
**comment**     | `String`      |              | UTF-8 строка + `\x00`                  | Комментарий (описание) к записи
**icon**        | `PUID`        | 16           | PUID[^2]                               | PUID[^2] иконки записи
...
Первое поле атрибута - это PUID[^2], оно наследуется от PUID[^2] того же атрибута у шаблона. Атрибут с нулевым[^4] PUID[^2] указывает на конец списка атрибутов. 
...
Название поля   | Тип           | Длина, байты | Метод хранения              | Описание
--------------- | ------------- | ------------ | --------------------------- | -----------------------------
**puid**        | `PUID`        | 16           | PUID[^2]                    | PUID[^2] вложения
...
Название поля   | Тип           | Длина, байты | Метод хранения                         | Описание
--------------- | ------------- | ------------ | -------------------------------------- | -----------------------------
**puid**        | `PUID`        | 16           | PUID[^2]                               | PUID[^2] иконки
...
[^2]: PUID *(**англ.** **P**iema **U**nique **Id**entifier)* - Уникальный идентификатор Piema
[^3]: UUID корзины - `21000000-0000-0000-0000-000000000000`
[^4]: Нулевой UUID - это `20000000-0000-0000-0000-000000000000`
[^5]: [3-ий вариант PUID](/puid/ru/README.md#основанный-на-времени-вариант-03)
